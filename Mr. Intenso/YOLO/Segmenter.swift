  ...
  override func processObservations(for request: VNRequest, error: Error?) {
    if let results = request.results as? [VNCoreMLFeatureValueObservation] {
      ...
      let detectedObjects = postProcessSegment(
        feature: pred, confidenceThreshold: super.settings?.segmentation ?? 0.0,
        iouThreshold: super.settings?.iou ?? 0.0)
      var boxes: [Box] = []
      var alphas = [CGFloat]()
      ...
      DispatchQueue.global(qos: .userInitiated).async {
        guard
          let procceessedMasks = generateCombinedMaskImage(
            ...
            threshold: super.settings?.segmentation ?? 0.0
        ...
        self.updateTime()
          self.currentOnResultsListener?.on(result: result, type: super.type!)
      }
    }
  }
  ...
  override func predictOnImage(image: CIImage) -> YOLOResult {
    ...
    do {
      ...
      if let results = request.results as? [VNCoreMLFeatureValueObservation] {
        ...
        let detectedObjects = postProcessSegment(
            feature: pred, confidenceThreshold: super.settings?.segmentation ?? 0.0, iouThreshold: super.settings?.iou ?? 0.0)
        var boxes: [Box] = []
        ...
        result.originalImage = UIImage(cgImage: cgImage)
